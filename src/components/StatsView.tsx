...
export default function StatsView({
  ...
  const getMemberStats = (member: Member) => {
    const present = allDates.filter(
      (d) => attendance[d]?.[member.id]
    ).length;
  ...
  const getGroupStats = (group: Group) => {
    ...
    if (groupMembers.length === 0) {
      return { avg: 0, members: [] as (Member & {
        present: number;
        pct: number;
      })[] };
    }

    const withStats = groupMembers.map((m) => ({
      ...m,
      ...getMemberStats(m),
    }));

    const avg = Math.round(
      withStats.reduce((s, m) => s + m.pct, 0) /
        withStats.length
    );

    return {
      avg,
      members: withStats.sort((a, b) => b.pct - a.pct),
    };
  };
    ...
    return (
      <div className="view">
        <div className="empty-state tall">
          ...
          <small>
            Gehe zu „Anwesenheit" und hake Mitglieder ab.
          </small>
        </div>
      </div>
    );
  ...
  return (
    <div className="view">
      ...
      {groups.map((group) => {
        const { avg, members: gm } =
          getGroupStats(group);

        return (
          <div key={group.id} className="stats-group">
            <div className="stats-group-header">
              <span className="stats-group-name">
                {group.name}
              </span>

              <span
                className="stats-avg-badge"
                style={
                  { "--pct": avg + "%" } as React.CSSProperties
                }
              >
                Ø {avg}%
              </span>
